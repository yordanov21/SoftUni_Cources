...
app.use(session({
    ...
    secret: ['skdjfdkshfsklf;alfhaslfksdf', 'sfghfghfhyuwerwer', 'asdasdeghfgjhgjhfgddfg'] //it can be only one or array of secrets
}))
...
app.get('/custom-cookie', (req, res) => {
    ...
    fs.readFile('./views/home.html', { encoding: 'utf-8' })
        .then(htmlResult => {
            res.send(htmlResult);
        });
});
...
app.get('/bcrypt/:password', (req, res) => {
    ...
    bcript.genSalt(saltRounds)
        ...
        .then(hash => {
            ...
            var filepasswordsDataPath = path.resolve(__dirname, './passwordsHolder.json');
            fs.writeFile(filepasswordsDataPath, JSON.stringify(passwordHolder, null, 4), (err) => {
                if (err) {
...
app.get('/bcrypt/verify/:password', (req, res) => {
    ...
    passwordHolder.forEach(element => {
        bcript.compare(req.params.password, element)
            .then(result => {
                console.log(result);
                if (result) {
                    isVerified = true;
